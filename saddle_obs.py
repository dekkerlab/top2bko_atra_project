#name of file saddle_obs with CT plots
...
#     )
    ...
    print(C)
    #Get the diagonal of saddle data for CT calculation
    ...
    #plt.bar(binedges, width=1/len(binedges), height=groupmean, align="edge", **margin_kws)
    plt.plot(binedges[:-1],diagonal)
    plt.xlim(lo, hi)
    #plt.ylim(0.5,2)  # correct
    ...
    plt.axvline(x=(lo + hi) / 2, linestyle='--', color='gray')
    plt.axhline(y=1, linestyle='--', color='gray')
    
    #Adding Compartment strength
    lg=len(ratiosAB['A'])
    #print("20 is:",np.floor(0.2*lg))
    A=ratiosAB['A']
    B=ratiosAB['B']
    #ax.text(binedges[10],binedges[10],np.around(B[int(np.floor(0.2*lg))],decimals=2),fontsize=12)
    #ax.text(binedges[-10],binedges[-10],np.around(A[int(np.floor(0.2*lg))],decimals=2),fontsize=12)
